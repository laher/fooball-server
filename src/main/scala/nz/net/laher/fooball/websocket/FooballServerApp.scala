  ...
  val routes = Routes({
    ...
    case WebSocketHandshake(wsHandshake) => wsHandshake match {
      ...
      case PathSegments("game" :: id :: Nil) => {
        val bcAddress= "/user/" + LobbyActor.actorRef + "/" + GameWSActor.broadcasterRefPart + id
        log.debug("Registering with broadcaster at {}", bcAddress)
        val broadcaster= actorSystem.actorFor(bcAddress)
        // To start Web Socket processing, we first have to authorize the handshake.
        // This is a security measure to make sure that web sockets can only be established at your specified end points.
        wsHandshake.authorize(onComplete = Some((event: WebSocketHandshakeEvent) => {
